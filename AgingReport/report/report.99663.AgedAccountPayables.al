report 99663 "3KT Aged Accounts Payable"
{
    DefaultLayout = RDLC;
    RDLCLayout = './AgingReport/layout/AgedAccountsPayable.rdl';
    ApplicationArea = Basic, Suite;
    Caption = '3KT Aged Accounts Payable';
    ...
    dataset
    {
        dataitem(Vendor; Vendor)
        {
            ...
            column(GrandTotalVLE10RemAmtLCY; GrandTotalVLERemaingAmtLCY[10])
            {
                AutoFormatType = 1;
            }
            ...
            dataitem(CurrencyLoop; "Integer")
            {
                ...
                dataitem(TempVendortLedgEntryLoop; "Integer")
                {
                    ...
                    column(AgedVendLedgEnt10RemAmtLCY; AgedVendorLedgEntry[10]."Remaining Amt. (LCY)")
                    {
                        AutoFormatType = 1;
                    }
                    ...
                    column(AgedVendLedgEnt10RemAmt; AgedVendorLedgEntry[10]."Remaining Amount")
                    {
        ...
        dataitem(CurrencyTotals; "Integer")
        {
            ...
            column(AgedVendLedgEnt10RemAmtLCY10; AgedVendorLedgEntry[10]."Remaining Amount")
            {
                AutoFormatExpression = CurrencyCode;
                AutoFormatType = 1;
            }
                ...
                if TempCurrencyAmount.FindSet(false) then
                    repeat
                        if TempCurrencyAmount.Date <> DMY2Date(31, 12, 9999) then
                            ...
                            AgedVendorLedgEntry[7]."Remaining Amount" := TempCurrencyAmount.Amount;
    ...
    protected var
        ...
        TotalVendorLedgEntry: array[10] of Record "Vendor Ledger Entry";
        AgedVendorLedgEntry: array[10] of Record "Vendor Ledger Entry";
        TempCurrency: Record Currency temporary;
        TempCurrency2: Record Currency temporary;
        TempCurrencyAmount: Record "Currency Amount" temporary;
        DetailedVendorLedgerEntry: Record "Detailed Vendor Ledg. Entry";
        PeriodLength: DateFormula;
        GrandTotalVLERemaingAmtLCY: array[10] of Decimal;
        ...
        agedAccpay: Report "Aged Accounts Payable";
    ...
    begin
        ...
        for i := 1 to NoOfPeriod do begin
            if not Evaluate(PeriodLength2, StrSubstNo(Text027, PeriodLengthValue[i])) then begin
        ...
        if AgingBy = AgingBy::"Due Date" then begin
            ...
            PeriodStartDate[1] := CalcDate(StrSubstNo(Text027, PeriodLengthValue[1]), EndingDate + 1);
        end;
        ...
        for i := 2 to ArrayLen(PeriodLengthValue) do begin
            if AgingBy = AgingBy::"Due Date" then begin
                if i <= NoOfPeriod + 1 then begin
                    ...
                    PeriodStartDate[i] := CalcDate(StrSubstNo(Text027, PeriodLengthValue[i - 1]), PeriodEndDate[i] + 1);
                end;
            end else begin
                if i <= NoOfPeriod then begin
                    PeriodEndDate[i] := PeriodStartDate[i - 1] - 1;
                    PeriodStartDate[i] := CalcDate(StrSubstNo(Text027, PeriodLengthValue[i]), PeriodEndDate[i] + 1);
                end;
            end;
        end;
        ...
        for i := 1 to ArrayLen(PeriodLengthValue) do begin
            ...
            if PeriodEndDate[i] < PeriodStartDate[i] then
                Error(Text010, PeriodLengthValue[i]);
        end;
    end;
