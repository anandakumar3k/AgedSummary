report 99665 "Stock Aging Qty-Value"
{
            ...
            dataitem(Item; Item)
            {
                ...
                dataitem(tempItemloop; "Integer")
                {
                    ...
                    dataitem("Integer"; "Integer")
                    {
                        ...
                        column(TotalValueEnt10RemAmt; TotalValueEntry[10]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt9RemAmt; TotalValueEntry[9]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt8RemAmt; TotalValueEntry[8]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt7RemAmt; TotalValueEntry[7]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt6RemAmt; TotalValueEntry[6]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt5RemAmt; TotalValueEntry[5]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt4RemAmt; TotalValueEntry[4]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt3RemAmt; TotalValueEntry[3]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt2RemAmt; TotalValueEntry[2]."Cost Amount (Actual)") { AutoFormatType = 1; }
                        column(TotalValueEnt1RemAmt; TotalValueEntry[1]."Cost Amount (Actual)") { AutoFormatType = 1; }

                        column(TotalValueEnt10RemQty; TotalValueEntry[10]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt9RemQty; TotalValueEntry[9]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt8RemQty; TotalValueEntry[8]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt7RemQty; TotalValueEntry[7]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt6RemQty; TotalValueEntry[6]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt5RemQty; TotalValueEntry[5]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt4RemQty; TotalValueEntry[4]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt3RemQty; TotalValueEntry[3]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt2RemQty; TotalValueEntry[2]."Valued Quantity") { AutoFormatType = 1; }
                        column(TotalValueEnt1RemQty; TotalValueEntry[1]."Valued Quantity") { AutoFormatType = 1; }
                        ...
                        begin
                            ...
                            // *** NEW: Value the remaining portion of this inbound layer as of EndingDate ***
                            RemainingValue := CalcRemainingValueAsOf(TempItemLedgerEntry, EndingDate, IncludeExpectedCost, TotalInvtQty);
                            ...
                            AgedValueEntry[PeriodIndex]."Cost Amount (Actual)" := RemainingValue;
                            TotalValueEntry[PeriodIndex]."Cost Amount (Actual)" += RemainingValue;
                            GrandTotalValueEntry[PeriodIndex]."Cost Amount (Actual)" += RemainingValue;
                ...
                begin
                    ...
                    // *** UPDATED: Item-level closing value respects IncludeExpectedCost and As-Of date ***
                    ValueEntry.Reset();
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    // *** NEW OPTION: Include Expected Costs ***
                    field(IncludeExpectedCost; IncludeExpectedCost)
                    {
                        ApplicationArea = All;
                        Caption = 'Include Expected Costs';
                        ToolTip = 'Include expected cost amounts when valuing inventory as of the Ending Date.';
                    }
    ...
    var
        ...
        UnitCost: Decimal; // left for backward compatibility (unused for valuation now)
    ...
    begin
        ...
        // AgingBy = Posting Date is the active mode in this implementation
        ItemLedEntry.SetRange("Posting Date", 0D, EndingDate);
    ...
    var
        ValueEntry: Record "Value Entry";
    begin
        // Deprecated: Value is now computed as-of EndingDate via CalcRemainingValueAsOf().
        UnitCost := 0;
    end;
    ...
    begin
        ...
        if AgingBy = AgingBy::"Manufacturer Date" then
            if Src."Manufacturer Date" in [0D .. EndingDate] then begin
                if not TempAtEndingDate.Get(EntryNo) then begin
                    TempAtEndingDate.Init();
                    TempAtEndingDate := Src;
                    TempAtEndingDate.Quantity := AppliedQty;
    ...
    // *** NEW: As-of valuation for remaining portion of an inbound layer ***
    local procedure CalcRemainingValueAsOf(var InILE: Record "Item Ledger Entry"; AsOfDate: Date; IncludeExp: Boolean; RemQty: Decimal): Decimal
    var
        VE: Record "Value Entry";
        TotalValueAsOf: Decimal;
        InQty: Decimal;
    begin
        VE.Reset();
        ...
        exit(Round(TotalValueAsOf * (RemQty / InQty), 0.01, '='));
    end;
}
